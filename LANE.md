...
You own the `copy-a` branch. Worktree `~/TarunsCode/bonsai-copy-a`, its own Neon database branch
already wired in `.env.local`. A parallel agent owns `copy-b` — no coordination, no copying, no
...
decide what the engine needs, use whatever connections, MCP servers, extensions, or workflows you
judge right. This file gives context, not instructions.
