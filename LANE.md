# LANE.md — copy-b (Codex lane)

You are the agent for the `copy-b` branch. Worktree `~/TarunsCode/bonsai-copy-b`, your own Neon
database branch (already wired in `.env.local` — never change `DATABASE_URL` to another lane's).
A parallel Claude Code agent runs the `copy-a` lane; you do not coordinate with it, copy from it,
or merge into it. Tarun compares the lanes and decides what reaches `main`.
...
- Candidate surfaces: **Chrome extension over claude.ai/chatgpt.com** (highest distribution,
  fragile DOM) and **CLI plugin / coding-agent integration** (real pain, specifiable). MCP was
  judged weak — a tree is client-side state the host UI won't render.
...
4. Build. Commit small, push to `origin/copy-b` often.
