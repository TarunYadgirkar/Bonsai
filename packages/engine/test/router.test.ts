...
import type { Effort, RoutingDecision, Tier } from '../src/types';
...
function routingFor(tier: Tier, model: string, effort: Effort): RoutingDecision {
  return {
    tier,
    model,
    effort,
...
describe('route — pinned tier', () => {
  ...
  it('reports complexity 2 for any non-deep pin', async () => {
    const { complete, calls } = fakeComplete([]);
    const d = await route({ question: 'Anything', contextTokens: 400, pinnedTier: 'quick' }, {
      complete,
    });
    expect(calls).toHaveLength(0);
    expect(d.tier).toBe('quick');
...
describe('route — auto mode', () => {
  it('classifies on the quick tier and routes complexity 3 to deep', async () => {
    const { complete, calls } = fakeComplete([
      llmResult('{"complexity": 3, "reason": "weighs trade-offs"}'),
    ]);
    const d = await route(
  ...
  it('parses JSON embedded in fenced output', async () => {
...
describe('answerFailsSanityCheck', () => {
  it('fails answers under 40 chars', () => {
    expect(answerFailsSanityCheck('Yes.')).toBe(true);
    expect(answerFailsSanityCheck('   short   ')).toBe(true);
  });

  it('fails punt phrases regardless of length', () => {
    expect(
...
describe('completeWithEscalation', () => {
  it('makes one call when the first answer passes', async () => {
    ...
    const out = await completeWithEscalation(
      {
        routing: routingFor('thoughtful', 'claude-sonnet-5', 'medium'),
        systemPrompt: 'sys',
        userPrompt: 'user q',
      },
      { complete },
  ...
  it('escalates a punting quick answer onto the next tier and sums costs', async () => {
    const { complete, calls } = fakeComplete([
    ...
    expect(out.routing.effort).toBe('medium');
    expect(out.routing.modelLabel).toBe('Sonnet 5');
  ...
  it('returns the punting first answer without a second call only at the ceiling model', async () => {
    const { complete, calls } = fakeComplete([
      llmResult(PUNT, {
        model: CEILING_MODEL,
        tier: 'deep',
